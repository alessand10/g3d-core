...
class CastCache {
    public:
    std::tuple<std::array<Ts, 256>...> caches;


    template <typename C>
    constexpr void cacheValue(C value, size_t index = 0) {
        std::get<std::array<C, 256>>(caches)[index] = value;
    }

    template <typename C>
    C getValue(size_t index = 0) {
        return std::get<std::array<C, 256>>(caches)[index];
    }
};
...
class BinAllocator {
    ...
    T* safeCast(void* ptr) {
        ...
        int index = (ptr - memoryBlock) >> 2;

        T* cacheLookup = m_castCache.template getValue<T*>((ptr - memoryBlock) >> 2);
        ...
        m_castCache.template cacheValue<T*>(reinterpret_cast<T*>(ptr), index);
